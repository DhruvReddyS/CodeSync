 ...
 */
async function fetchContributionStreaks(username) {
        ...
        for (const day of days) {
            ...
            if (day.count > 0) {
                ...
                if (!prevDate) {
                    ...
                    const diffDays = (currDate.getTime() - prevDate.getTime()) /
                        (1000 * 60 * 60 * 24);
                    if (diffDays === 1) {
                        runningStreak += 1;
                    }
...
async function fetchPinnedRepositories(username) {
  ...
  `;
    try {
        const res = await axios_1.default.post("https://api.github.com/graphql", { query, variables: { username } }, {
            headers: {
