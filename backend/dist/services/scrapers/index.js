...
exports.scrapePlatformForUser = scrapePlatformForUser;
exports.scrapeAllPlatformsForUser = scrapeAllPlatformsForUser;
 ...
 * ------------------------------------------------------------------ */
function mapLeetCode(stats) {
    const badgeCount = typeof stats.badges === "number" ? stats.badges : 0;
...
function mapHackerRankBadgeLevel(level) {
    if (typeof level === "string") {
        ...
        if (lower.includes("legend"))
            return "legendary"; // requires BadgeLevel union to include "legendary"
...
function mapHackerRank(stats) {
    ...
    return {
        ...
        certificatesCount: Array.isArray(stats.certificates)
            ? stats.certificates.length
            : stats.certificatesCount ?? 0,
    };
}
function mapGitHub(stats) {
 ...
 * ------------------------------------------------------------------ */
async function scrapePlatformForUser(platform, handle) {
    if (!handle)
        return null;
    try {
        switch (platform) {
            case "leetcode": {
                const raw = await (0, leetcodeScraper_1.scrapeLeetCode)(handle);
                return raw ? mapLeetCode(raw) : null;
            }
            case "codeforces": {
                const raw = await (0, codeforcesScraper_1.scrapeCodeforces)(handle);
                return raw ? mapCodeforces(raw) : null;
            }
            case "codechef": {
                const raw = await (0, codechefScraper_1.scrapeCodeChef)(handle);
                return raw ? mapCodeChef(raw) : null;
            }
            case "atcoder": {
                const raw = await (0, atcoderScraper_1.scrapeAtcoder)(handle);
                return raw ? mapAtcoder(raw) : null;
            }
            case "hackerrank": {
                const raw = await (0, hackerrankScraper_1.scrapeHackerRank)(handle);
                return raw ? mapHackerRank(raw) : null;
            }
            case "github": {
                const raw = await (0, githubScraper_1.scrapeGitHub)(handle);
        ...
        console.error(`[SCRAPER] Failed platform=${platform}, handle=${handle}:`, err?.message || err);
        return null;
    }
}
async function scrapeAllPlatformsForUser(handles) {
    const jobs = [];
    if (handles.leetcode)
