...
const { students: studentsCol, studentScores: studentScoresCol } = collections_1.collections;
/* ----------------- utils ----------------- */
...
function withinLastDays(isoOrMillisOrTs, days) {
...
function computeSignal(platform, st) {
    if (!st)
        return 0;
    switch (platform) {
        case "leetcode": {
...
function computePlatformNumbers(platform, st) {
    if (!st)
        return null;
...
function computePlatformScoreBreakdown(platform, st) {
    if (!st)
...
router.get("/students", auth_middleware_1.default, role_middleware_1.requireInstructor, async (req, res) => {
    try {
        ...
        let queryRef = studentsCol;
        if (branch)
            queryRef = queryRef.where("branch", "==", branch);
        if (section)
            queryRef = queryRef.where("section", "==", section);
        if (year)
            queryRef = queryRef.where("yearOfStudy", "==", year);
        ...
        const students = await Promise.all(snap.docs.map(async (doc) => {
            ...
            const cpScores = scoresDoc.exists ? scoresDoc.data() : {};
            const score = isNum(cpScores?.displayScore)
                ? clamp(cpScores.displayScore)
                : 0;
            const lastActiveRaw = d.lastActiveAt ?? d.updatedAt ?? null;
            return {
                id: doc.id,
            ...
            if (!q)
                return true;
            const hay = `${s.name} ${s.id} ${s.rollNumber ?? ""} ${s.branch ?? ""} ${s.section ?? ""} ${s.year ?? ""}`.toLowerCase();
 ...
 * ================================================== */
router.get("/students-directory", auth_middleware_1.default, async (req, res) => {
    try {
        ...
        const students = snap.docs
            ...
            const score = isNum(cpScores.displayScore)
                ? clamp(cpScores.displayScore)
                : 0;
            const lastActiveRaw = d.lastActiveAt ?? d.updatedAt ?? cpScores.updatedAt ?? null;
            const platformSignals = d.platformSignals ??
                d.platforms ?? {
                leetcode: 0,
                codeforces: 0,
                codechef: 0,
 ...
 * ✅ Instructor-only
 * ================================================== */
router.get("/dashboard", auth_middleware_1.default, role_middleware_1.requireInstructor, async (req, res) => {
    try {
        const branch = req.query.branch?.trim();
        const section = req.query.section?.trim();
        const year = req.query.year?.trim();
        ...
        const students = snap.docs
            ...
            if (!q)
                return true;
            const hay = `${s.name} ${s.id} ${s.branch ?? ""} ${s.section ?? ""} ${s.year ?? ""} ${s.rollNumber ?? ""}`.toLowerCase();
            return hay.includes(q);
        });
        return res.json({
            students,
 ...
 * ✅ Instructor-only
 * ================================================== */
router.get("/student/:id/stats", auth_middleware_1.default, role_middleware_1.requireInstructor, async (req, res) => {
    try {
 ...
 * ✅ Instructor-only
 * ================================================== */
router.post("/refresh-cohort", auth_middleware_1.default, role_middleware_1.requireInstructor, async (_req, res) => {
