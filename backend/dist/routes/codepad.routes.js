...
const RAW_KEYS = process.env.GEMINI_KEYS || process.env.GEMINI_API_KEY || process.env.GEMINI_KEY || "";
...
const GEMINI_CLIENTS = GEMINI_KEYS.map((key) => new generative_ai_1.GoogleGenerativeAI(key));
/* ------------------------------------------------
...
function isRetryableGeminiError(err) {
    ...
    return (status === 429 || // quota / rate limit
...
router.post("/ai-helper", async (req, res) => {
    ...
    const { language, version, code, input, programOutput, programError, } = req.body || {};
    if (!language || !code) {
        return res
            .status(400)
